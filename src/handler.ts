...
export async function handleRequest(request: Request, env: Env): Promise<Response> {
	...
	if (!keyResult.valid) {
		...
		return errorResponse(messageMap[keyResult.code], keyResult.code, statusMap[keyResult.code]);
	}
	...
	// Fetch the prompt
	const promptResult = await fetchPrompt(env, promptId, keyResult.organizationId, version);

	if ('error' in promptResult) {
		const status = promptResult.code === 'NOT_FOUND' ? 404 : 404;
